...
A prática foi desenvolvida por meio de uma abordagem quantitativa e descritiva, utilizando o ambiente R e o pacote leem para a organização, representação e análise dos dados.

Inicialmente, foi elaborado um banco de dados contendo 25 resultados simulados de resistência à compressão do concreto, expressos em MPa. Os dados foram definidos considerando condições experimentais constantes: relação água/cimento igual a 0,50, idade de 28 dias e corpos de prova cilíndricos com 100 mm de diâmetro e 200 mm de altura.

Como os dados utilizados são simulados, não foram realizados efetivamente ensaios laboratoriais. Os valores foram construídos de forma plausível para representar a variabilidade que poderia ser observada em corpos de prova submetidos a condições semelhantes.

O procedimento de análise foi dividido nas seguintes etapas:

1.	Organização dos dados: inserção dos 25 resultados no ambiente R;

2.	Classificação da variável: identificação da resistência à compressão como variável quantitativa contínua;

3.	Tabulação: agrupamento dos resultados em classes de frequência;

4.	Definição das classes: considerando (m = 25), foi adotado o critério (k \approx \sqrt{m}), resultando em cinco classes;

5.	Cálculo das frequências: determinação das frequências absoluta, relativa, percentual e acumuladas;

6.	Representação gráfica: elaboração de histograma para visualização da distribuição dos resultados;

7.	Análise estatística: aplicação das medidas de tendência central e dispersão, além dos coeficientes de assimetria e curtose;

8.	Interpretação: comparação dos resultados estatísticos e discussão de sua relação com a variabilidade da resistência à compressão do concreto.
Para a execução dos procedimentos computacionais, foi utilizado o pacote leem, juntamente com recursos de apresentação tabular e gráfica disponíveis no R. O objetivo foi utilizar o software não apenas para obter os valores calculados, mas também para compreender e interpretar os procedimentos estatísticos empregados.
...
```{r}
#| label: tbl-dados
#| tbl-cap: "Resultados simulados de resistência à compressão do concreto aos 28 dias"
#| echo: false
...
kable(
  resistencia25,
  col.names = c("Corpo de Prova", "Resistência à compressão (MPa)"),
  align = c("c", "c"),
  digits = 1
)
```
...
```
```{r}
#| label: tbl-frequencia-resistencia
#| tbl-cap: "Distribuição de frequência da resistência à compressão"
...
Conforme apresentado na Tabela @tbl-frequencia-resistencia, observa-se que a maior frequência de resistência ocorre na classe de 40,08 a 41,00 MPa, que concentra 8 dos 25 ensaios realizados, correspondendo a 32% das observações.
