...
* **`read_delim()`**: Permite importar arquivos de texto delimitados, especificando explicitamente o caractere utilizado para separar as colunas. Neste relatório, foi utilizado o ponto e vírgula (;) como delimitador.

* `locale()`: Permite configurar convenções utilizadas durante a importação dos dados, como o separador decimal. Foi utilizado para indicar o ponto (.) como separador decimal da base analisada.
...
* `glimpse()`: Apresenta uma visão compacta da estrutura da base de dados, permitindo verificar as variáveis, seus tipos e alguns de seus valores.

* `slice_head()` e `slice_tail()`: Permitem selecionar, respectivamente, as primeiras e as últimas observações de uma base de dados, sendo úteis na inspeção inicial dos registros.

* `count()`: Realiza a contagem das observações de acordo com uma ou mais variáveis, facilitando a identificação da frequência das categorias.


* `distinct()`: Retorna combinações ou valores únicos de uma ou mais variáveis, sendo útil para identificar categorias existentes e possíveis inconsistências de padronização.

* `ungroup()`: Remove agrupamentos previamente estabelecidos por group_by(), evitando que operações posteriores continuem sendo executadas separadamente por grupo.

* `n()`: Utilizada dentro de operações de resumo para determinar o número de observações existentes em cada grupo.
...
* `str_squish()`: Remove espaços em branco no início e no final dos textos e reduz sequências de múltiplos espaços internos a um único espaço, auxiliando na padronização das categorias.

* `regex()`: Permite definir expressões regulares utilizadas na busca por padrões textuais, incluindo opções como ignorar diferenças entre letras maiúsculas e minúsculas.
...
* `if_any()`: Verifica se uma determinada condição é verdadeira em pelo menos uma das colunas selecionadas. Pode ser utilizada para localizar observações que apresentam valores ausentes ou inconsistências em qualquer uma das variáveis investigadas.
...
* `geom_smooth()`: Adiciona ao gráfico uma curva ou linha de tendência. 

* `geom_line()`: Conecta observações por meio de linhas, facilitando a visualização da evolução ou tendência de uma variável ao longo de outra, como na relação entre idade e resistência média.

* `labs()`: Define elementos textuais do gráfico, como título e rótulos dos eixos.

* `theme_minimal()`: Aplica ao gráfico um tema visual simplificado, reduzindo elementos gráficos desnecessários e favorecendo a leitura dos dados.
...
dados |>
  slice_head(n = 6) # Visualiza as seis primeiras linhas 

dados |>
  slice_tail(n = 10) # Visualiza as dez útlimas linhas

cat(
  "\nA base de dados possui",
  dim(dados)[1], "observações e",
  dim(dados)[2], "variáveis.\n"
)
```
...
```{r}
# 8: Analisando erros de digitação
# filter(): Seleciona as observações (linhas) suspeitas.
# if_any(): Varre múltiplas colunas verificando se 'qualquer' uma contém o erro.
# -(1:3): Exclui as 3 primeiras colunas (id, obra, tipo) da varredura, pois são categóricas e naturalmente contêm letras e hifens.
...
```{r}
# 16: Verificação final da estrutura
...
# 20: Estatísticas por grupo (Múltiplas Variáveis)
...
```{r}
# 21: Resistência relativa - A resistência de referência é a resistência característica (fck) que dá nome à classe do concreto (ex: C25 exige referência de 25 MPa). Utilizamos a função case_when() para mapear cada classe ao seu respectivo valor e, em seguida, calculamos a razão entre o valor ensaiado e essa referência.
...
```{r}
# 22: Classificação de corpos de prova
...
```{r}
# 23: Variável acima_media - Para calcular se a amostra está acima da média de sua classe sem precisar calcular os valores manualmente, combinamos group_by() com mutate(). Isso força a função mean() a calcular a média internamente para cada bloco de concreto.
...
```{r}
# 24: Contagem de observações por grupo
...
```{r}
# 26: Resumo estatístico descritivo simultâneo
...
```{r}
# 27: Seleção de variáveis quantitativas
...
# 28: Média simultânea com across()
...
```{r}
# 29: Transformação simultânea (mutate + across)
...
# 30: Múltiplas estatísticas com across()
...
```{r}
# 31: Verificação e padronização da variável obra
...
```{r}
# 32: Localização de padrões textuais
...
```{r}
# 33: Distribuição da resistência à compressão por tipo de concreto
...
```{r}
# 34: Relação entre consumo de cimento e resistência à compressão
...
```{r}
# 35: Relação entre relação água/cimento e resistência à compressão
# ggplot(): Define a base de dados e os mapeamentos do gráfico.
# geom_point(): Representa cada corpo de prova como um ponto no gráfico.
# geom_smooth(): Adiciona uma linha de tendência para facilitar a identificação
...
```{r}
# 36: Comparação do desempenho dos concretos entre as obras
...
```{r}
# 37: Relação entre idade e resistência à compressão
...
```{r}
# 38: Tabela-resumo para acompanhamento da qualidade do concreto
